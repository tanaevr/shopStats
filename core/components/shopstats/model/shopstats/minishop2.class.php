...
class minishop2_shop implements statsInterface
{
    ...
    function __construct(shopStats & $stats, array $config = [])
    {
        $this->stats = &$stats;
    ...
    function getStats()
    {
        ...
        $q_status = $this->modx->newQuery('msOrderStatus', ['active' => 1]);
        $q_status->select('id,name,color');
        if ($q_status->prepare() && $q_status->stmt->execute()) {
            while ($row = $q_status->stmt->fetch(PDO::FETCH_ASSOC)) {
                //$output[$row['id']] = $row;
                $output['total_counts'][$row['id']] = [
                    'name' => $row['name'],
                    'color' => $row['color'],
                    'count_orders' => $this->modx->getCount('msOrder', ['status' => $row['id']]),
                ];
            }
        }
        ...
        if ($q_stats_month->prepare() && $q_stats_month->stmt->execute()) {
            ...
            while ($row = $q_stats_month->stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                $output['stats_month'][$date['year'] . '-' . $date['month']][$row['status']] = [
            ...
            $output['users_count'] = $this->modx->getCount('modUser', ['active' => 1, 'primary_group' => 0]);
        }
